...
impl Mandelbrot {
    ...
    #[inline]//But this is okay
    #[target_feature(enable = "sse2")]
    unsafe fn mandelbrot_iterations_sse2(self: &Self, c_real_f: [Nice128D; 2], c_imag_f: [Nice128D; 2]) -> [Nice128I; 2] {//Returns MAX_ITERATIONS if it is bounded
        ...
        for _ in 0..self.max_iterations {
            ...
            let compare_i = [squared_sum_f[0].set_u64s_in_nice128i_if_less_than(diverge_threshold_squared_f), squared_sum_f[1].set_u64s_in_nice128i_if_less_than(diverge_threshold_squared_f)];
            ...
            //If both complex numbers have diverged (entire vector is 0), return
            if (compare_i[0].create_mask_from_msbs_of_u8s() == 0) && (compare_i[1].create_mask_from_msbs_of_u8s() == 0) {
                break;
            }

            //Increment the corresponding count only if we haven't converged yet
            incrementor_i = [compare_i[0] & incrementor_i[0], compare_i[1] & incrementor_i[1]];//If a number diverged, never increment its iteration count again
    ...
    unsafe fn update_sse2(self: &mut Self) {
        ...
        let real_step_amount_vector = Nice128D::with_f64s_all_set_to(real_step_amount * 4.0);//x4 since we process four at a time
        let imag_step_amount_vector = Nice128D::with_f64s_all_set_to(imag_step_amount);

        let mut c_real = [Nice128D::with_f64s(self.min_real + (real_step_amount * 3.0), self.min_real + (real_step_amount * 2.0)), Nice128D::with_f64s(self.min_real + real_step_amount, self.min_real)];
        for x in (0..self.x_samples).step_by(4) {
            let mut c_imag = [Nice128D::with_f64s_all_set_to(self.min_imag); 2];
            for y in 0..self.y_samples {
                ...
                result[1].store_u64s_unaligned_to(pointer);
                result[0].store_u64s_unaligned_to(pointer.offset(2));
